...
- **Backend API** — при `npm run dev` запросы к `/api` и `/assets` проксируются на `http://localhost:8000` (см. `vite.config.ts`)
...
│   │   │   ├── BottomPanel/       # Нижняя панель (чаты, новый чат)
│   │   │   ├── ChatArea/          # Область чата
│   │   │   ├── ChatFilesModal/    # Модалка файлов чата
│   │   │   ├── Header/            # Шапка (модель, язык, уведомления)
│   │   │   ├── NotesPanel/       # Панель заметок
│   │   │   ├── NotificationPanel/ # Уведомления
│   │   │   ├── PanelToggle/      # Переключатель sidebar / bottom
│   │   │   ├── SearchPanel/      # Глобальный поиск
│   │   │   ├── Sidebar/          # Боковая панель и список чатов
│   │   │   └── SupportPanel/     # Поддержка и статьи
│   │   └── ui/            # UI компоненты
│   │       ├── Button/          # Кнопка
│   │       ├── Icon/             # Иконка
│   │       └── Input/            # Поле ввода
...
│   │   ├── SpacesListPage/       # Список пространств (маршрут /spaces)
...
│   │   └── theme.ts              # Тема (light / dark / system)
...
*В дереве исходников может оставаться каталог `pages/SpacesPage/` — в текущем `App.tsx` для маршрута `/spaces` используется `SpacesListPage`.*
...
- Просмотр чатов/заметок/тегов; отправка сообщений в публичный чат — если включено API (`POST /api/public/.../chat/send`)
- Отображает данные пространства по публичному токену
