    ...
    def transcribe_audio(self, audio_bytes: bytes, filename: str = "audio.webm", language: str = "ru") -> str:
        ...
        if self.local_whisper:
            try:
                ...
                if not self.local_whisper.is_ready():
                    print("⚠️ Модель Whisper еще не загружена")
                    # Если есть API, сразу переключаемся на него
                    if self.whisper_client:
                        print("🔄 Модель не готова, используем Whisper API...")
                        ...
                        raise ValueError(
                            ...
                            "Пожалуйста, подождите несколько минут после запуска приложения. "
                            "Или используйте Whisper API, установив USE_WHISPER_API=true и OPENAI_API_KEY."
                        )
                
                # Модель готова, выполняем транскрибацию с таймаутом
                import threading
                ...
                thread.start()
                thread.join(timeout=120)  # Увеличиваем таймаут до 120 секунд для транскрибации
                
                if thread.is_alive():
                    print("⏱️ Транскрибация превысила таймаут (120 сек), переключаемся на API...")
