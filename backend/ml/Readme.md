...
│   ├── __init__.py
│   ├── dataset_generation.py      # Основной датасет (включая класс graphic)
│   └── graphic_types_dataset.py   # Доп. примеры для категории graphic
...
- Классификация текстовых вопросов по **7 категориям** (включая `graphic`)
...
- `__init__()` - Инициализация классификатора
  ...
  - Категории: `marketing`, `finance`, `legal`, `management`, `sales`, `general`, `graphic`
...
- `calculate_keyword_features(text: str) -> np.array` - Извлечение признаков ключевых слов
  ...
  - Возвращает массив из 21 признака (7 категорий × 3 типа признаков)
...
7. **graphic** - Запросы на графики, диаграммы и визуализацию данных
...
**Содержимое:**
- `business_dataset` — объединённый список примеров (фактический размер см. через `len(business_dataset)`; порядка **690+** записей, 7 классов)
- Распределение по классам почти равномерное; точные числа выводит `train_classifier.py` / `Counter`
...
**Параметры модели (как в коде `business_classifier.py`):**
...
1. **Семантические эмбеддинги** — в текущей реализации подаётся **матрица нулей** заданной размерности (384), чтобы сохранить совместимость формы признаков; реальный SentenceTransformer не подключён.

2. **Признаки ключевых слов** (21 признак = 7 категорий × 3 метрики)
   - Presence, TF, нормализованный счёт по ключевым словам

3. **Текстовые метрики** (5 признаков)

**Итоговая размерность вектора признаков:** 384 + 21 + 5 = **410** (эмбеддинги-заглушка + лексика + метрики)
...
**Связь с LLM:** предсказанная категория и вероятности используются в `chat_routes` для выбора доменного промпта (`CATEGORY_PROMPTS`); генерация ответа выполняется выбранным LLM-провайдером, а не этим RandomForest.
